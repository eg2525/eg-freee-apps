...
def app1():
    ...
    st.markdown("""
        ### R4での出力手順
        1. 帳表から「合計残高試算表」を選択
        2. 条件設定が「出力形式：通常/出力区分：累計/金額：円」であることを確認
        3. 出力設定は「表紙：しない/貸借対照表：する/損益計算書：しない/原価報告書：しない」を選択
        4. 実行ではなくヘッダー右側の「テキスト(CF12)」から.txtファイルで出力してください。
        """)
    ...
    if uploaded_file is not None:
        try:
            ...
            # 除外したい勘定科目コードを含む行を除外
            exclude_codes = [9025, 9045, 9055, 9075, 9145, 9150, 9200, 9210,9230, 9240, 9300, 9350, 9370, 9400, 9440, 9445, 9450, 9500]
